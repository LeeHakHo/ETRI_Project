...
def eval(cfg, args,model_path):
    ...
    for i,data_name in enumerate(cfg.test.test_list):
        ...
        gt_file = os.path.join(cfg.test.save_dir, 'gt.txt')
        pred_file = os.path.join(cfg.test.save_dir, 'submit.txt')
        name_file = os.path.join(cfg.test.save_dir, 'name.txt')
...
def main():
    ...
    result_path = os.path.join(cfg.test.save_dir, 'result.csv')
