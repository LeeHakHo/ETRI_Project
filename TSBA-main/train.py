        ...
        try:
            ...
            if opt.lgmix is True:
                ...
                for ml, sl in zip(labels, sub_labels):
                    cf = context_feature[:, :int(len(ml)//2), :]
                    sub_cf = sub_context_feature[:, int(len(sl)//2):, :]
                ...
                ms_cost = criterion(preds.view(-1, ms_preds.shape[-1]), ms_target.contiguous().view(-1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lgmix', type=bool, default=True) #Leehahko
