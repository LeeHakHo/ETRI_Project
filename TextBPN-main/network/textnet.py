...
class Evolution(nn.Module):
    ...
    def get_boundary_proposal(input=None, seg_preds=None, switch="gt"):
        ...
        if switch == "gt":
            ...
            #inds = inds.to(input["img"].device)  # LeeHakho
            #init_polys = init_polys.to(input["img"].device) #LeeHakho
        else:
    ...
    @staticmethod
    def get_boundary_proposal_eval(input=None, seg_preds=None, switch=None):
    ...
    def forward(self, cnn_feature, input=None, seg_preds=None, switch="gt"):
        ...
        if self.is_training:
            ...
            if cfg.CRAFT is True:
                init_polys, inds = self.get_boundary_proposal_eval(input=input, seg_preds=seg_preds, switch ="gt")
            else:
                init_polys, inds = self.get_boundary_proposal_eval(input=input, seg_preds=seg_preds, switch=None)
            if init_polys.shape[0] == 0:
                return [init_polys for i in range(self.iter)], init_polys, inds
