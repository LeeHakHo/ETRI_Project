...
def get_node_feature(cnn_feature, img_poly, ind, h, w):
    ...
    batch_size = cnn_feature.size(0)
    #print(img_poly.size(1))
    gcn_feature = torch.zeros([img_poly.size(0), cnn_feature.size(1), img_poly.size(1)]).to(img_poly.device)
    for i in range(batch_size):
        #poly = img_poly[ind == i].unsqueeze(0)

        poly = img_poly[ind == i]
        poly = poly.squeeze(1)  # LeeHakho
        poly = poly.unsqueeze(0)
        output = torch.nn.functional.grid_sample(cnn_feature[i:i + 1], poly.cuda())
        gcn_feature[ind == i] = output[0].permute(1, 0, 2)
    return gcn_feature
