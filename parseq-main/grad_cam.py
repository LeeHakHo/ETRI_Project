...
input_tensor = normalize(resize(img, (32, 128)) / 255., [0.485, 0.456, 0.406], [0.229, 0.224, 0.225]).cuda()
#input_tensor = img.Normalize(0.5, 0.5)
model = model.encoder
print(model)
#target_layers = [model.encoder.blocks[-1].norm1]
target_layers = [model.blocks[-1].norm1]

from torchcam.methods import SmoothGradCAMpp, GradCAMpp, LayerCAM, XGradCAM, CAM, GradCAMpp, SmoothGradCAMpp, ScoreCAM, SSCAM, ISCAM, XGradCAM

cam_extractor = ScoreCAM(model, target_layer= target_layers,input_shape=(3,32,128))
#SmoothGradCAMpp.compute_cams.class_idx += 1

# Preprocess your data and feed it to the model
out = model(input_tensor.unsqueeze(0)).cuda()
out_num = out.squeeze(0).argmax().item()
print(out_num)
print(out.shape)
...
# target_layers = [model.encoder.blocks[-1].norm1]
# img = read_image("/home/ohh/PycharmProject/parseq-main/demo_images/cc.png")
# input_tensor = normalize(resize(img, (32, 128)) / 255., [0.5, 0.5, 0.5], [0.225, 0.225, 0.225])
...
# visualization = show_cam_on_image(to_pil_image(img), grayscale_cam, use_rgb=True)